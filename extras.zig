...
pub fn reduceNumber(alloc: std.mem.Allocator, input: u64, comptime unit: u64, comptime base: string, comptime prefixes: string) !string {
    ...
    const numerator: f64 = @floatFromInt(input);
    const denominator: f64 = @floatFromInt(input);
    return try std.fmt.allocPrint(alloc, "{d:.3} {s}{s}", .{ numerator / denominator, prefixes[exp .. exp + 1], base });
}
